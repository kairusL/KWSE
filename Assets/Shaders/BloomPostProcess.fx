...
{
	float3 baseColor = baseTextureMap.Sample(textureSampler, input.texCoord).rgb;
	//float4 bloomColor = bloomTextureMap.Sample(textureSampler, input.texCoord);
	//return baseColor + bloomColor;

	float3 screenColor = (float)bloomTextureMap.Sample(textureSampler, input.texCoord).rgb+ baseColor;
	...
	if (uv.x < (vx_offset - 0.005))
	{
		...
		float2 coord = float2(dx*floor(uv.x / dx),
			dy*floor(uv.y / dy));
		tc = (float3)bloomTextureMap.Sample(textureSampler, coord).rgb+ (float3)baseTextureMap.Sample(textureSampler, coord).rgb;
	}
