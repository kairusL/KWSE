...
void GameState::RenderDepthMap()
{
	...
	auto matWorld = Matrix4::Translation({ mSriFiPosition.x,mSriFiPosition.y,mSriFiPosition.z }) *KWSE::Math::Matrix4::RotationX(mRotation.x) * Matrix4::RotationY(mRotation.y) *Matrix4::RotationZ(mRotation.z);
...
void GameState::RenderScene()
{
	...
	// SciFi
	matWorld = Matrix4::Translation({ mSriFiPosition.x,mSriFiPosition.y,mSriFiPosition.z }) *KWSE::Math::Matrix4::RotationX(mRotation.x) * Matrix4::RotationY(mRotation.y) *Matrix4::RotationZ(mRotation.z);
	data.wvp[0] = Transpose(matWorld*(matView)*matProj);
	data.wvp[1] = Transpose((matViewLight)*matProjLight);
	mTransformBuffer.Update(data);
	mSci_fi_Texrures.BindPS(0);
